...
def we_need_to_talk_about_scp_055():
    return """"Can I smoke?"
...
def unforgettable_thats_what_you_are():
    return """"El, it's finished."
...
Footnotes
1. Excerpt from Document 180047109-L4799-098, User's Guide To Chemical Mnestics:
...
Not-Kim lets its keycard fall from its fingers and scoops up the ray gun. For a moment it contemplates the unconscious Wheeler, then examines the gun itself, remembering how it works. It turns back to face the airlock and fires, punching fat cylindrical holes in the white metal of the inner door until it's gone, then the outer door too, breaching the hermetic seal. A faint smile returns to not-Kim's face as SCP-3125 and its familiar, comforting signals flood into the bunker.
...
def interlude_scp2256():
    return """Item #: SCP-2256
...
- Addendum 3:
    ...
    Site 167 is a non-entity, of course, which is most likely why you missed it. It's likely ruined, and it's quite definitely a long God-damned way from here on foot. But still! I think it's better than half a chance.
...
def where_have_you_been_all_my_life():
    return """Who the fuck infiltrates a senior Foundation official's home, while they're home?
...
def ojai():
    return """Foundation Agent George Barsin is monolithic: nearly two metres tall and rectangular-shouldered, like a Bruce Timm cartoon. He is bald, bearded, and immaculately presented. His suit is tailored; there are few which will fit him off the rack.
...
def wild_light():
    return """The meeting room is Containment Unit S167-00-1006, which is the skull of a stillborn Cryptomorpha gigantes.
...
def blood_brain():
    return """There's no day/night cycle.
...
def tombstone():
    return """All memetic horror aside, Wheeler thought Site 41 seemed like a pleasant enough place to work, at least above ground. Decently spacious, if unattractive, offices; large windows, plenty of natural light, scenic forest views. Safe.
...
def champions_of_nothing():
    return """"…And what have we learned?"
...
def reader():
    chapters = [
        ("SCP-055", SCP_55()),
        ("We Need to Talk About SCP-055", we_need_to_talk_about_scp_055()),
        ("Introductory Antimemetics", introductory_antimemetics()),
        ("Your Last First Day", your_last_first_day()),
        ("Interlude: SCP-2256", interlude_scp2256()),
        ("SCP-3125", SCP_3125()),
        ("Where Have You Been All My Life?", where_have_you_been_all_my_life()),
        ("Ojai", ojai()),
        ("Fresh Hell", fresh_hell()),
        ("Immemorial", immemorial()),
        ("Wild Light", wild_light()),
        ("Unforgettable", unthreaded()),
        ("Blood-Brain Barrier", blood_brain()),
        ("Ara Orún", ara_orun()),
        ("Tombstone", tombstone()),
        ("Champions of Nothing", champions_of_nothing()),
    ]
    
    current = 0
    
    while True:
        title, content = chapters[current]
        
        # Use less to display content
        ...
        if cmd == 'q':
            break
        elif cmd == 'n' and current < len(chapters) - 1:
            current += 1
